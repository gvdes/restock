<template>
  <q-page padding>
    <div class="text-center text-h6 text-bold">Reporte {{ typeof (fechas) == 'object' ? `Del ${fechas.from} a
      ${fechas.to}` : fechas }}</div>
    ...
    <div class="row">
      ...
      <q-separator spaced inset vertical dark />
      <q-select v-model="workpoints.val" :options="workpoints.workpoints" label="Sucursal" class="col"
...
const init = async () => {
  ...
  fechas.value = fecha
  console.log(fecha);
  const resp = await AssitApi.getStores()
  if (resp.status == 200) {
...
const getData = async () => {
  ...
  console.log(data);
  const resp = await AssitApi.getData(data);
  console.log(resp)
  if (resp.status == 200) {
    console.log(resp.data);

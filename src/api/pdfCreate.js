...
const pdfTransfer = async (data, qrvalue, pedido) => {
  // console.log(data.folio);
  let sal = {
    traspaso: data
  }
  const currentDate = new Date();
  let dat = await AssitApi.getTransfer(sal);
  console.log(dat);
  if (dat.status == 200) {
    const qrData = `http://192.168.10.189:2200/#/checkin/${pedido}?key=${qrvalue}`;
    // const qrData = `http://192.168.10.112:9000/#/checkin/${pedido}?key=${qrvalue}`;
    ...
    for (let i = 0; i < 2; i++) {
      ...
      chunks.forEach(function (chunk, index) {
        ...
        // doc.text('LLUVIA LIGTH SA DE CV', 120, 25, 'left')
        doc.setFontSize(8)
        doc.text(dat.data.heades.DOMEMP, 10, 30, 'left')
        doc.text(`${dat.data.heades.CPOEMP} ${dat.data.heades.POBEMP}`, 10, 35, 'left')
        ...
        doc.text(dat.data.traspaso.TRASPASO, 32, 60, 'left')
        doc.text('PAGINA', 54, 55, 'left')
        doc.text(`${index + 1} de ${paginas}`, 54, 60, 'left')
        doc.text('FECHA', 76, 55, 'left')
        const fecha = dayjs(dat.data.traspaso.FECHA).format("YYYY-MM-DD")
        ...
        autoTable(doc, {
          ...
          body: [
            [ `(${dat.data.traspaso.ALMOR}) ${dat.data.traspaso.ALDMOR}`, `(${dat.data.traspaso.ALMDES}) ${dat.data.traspaso.ALDDES}`],
          ],
        })
    ...
    doc.save(`Traspaso${dat.data.traspaso.TRASPASO}`)
